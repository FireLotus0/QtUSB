...
基于 Qt 封装的 libusb，最大程度简化常规USB数据传输，使用者***不需要关心设备创建，接口声明，设备释放等***一些列操作，***唯一需要***的是：
1. 添加目标设备PID，VID，即可实现热插拔监听
2. 设备到来时，通过PID，VID获取到设备，使用接口read，write即可进行数据传输，数据传输均是异步进行
3. 设备断开时，不需要关心设备的销毁和释放
...
描述了设备使用的配置信息：配置，接口，端点。由于传输方式(控制，中断，批量)是由端点描述符决定的，所以在用户获取到UsbDevice时，***必须调用setConfiguration
进行设置，这样用户层面只需使用read,write即可，不需要关心具体使用哪种传输方式***，库内部会根据配置自动选择。对于***readCacheSize***，在
以往的开发中，***读取缓冲区大小对传输速度具有重要的影响***，因此提供该参数由用户自己进行设置。而queuedCommands，则是为了方便实现一些应用层面上的处理
...
### 2. UsbDevManager
这是一个单例类。代替用户进行USB设备管理：进行热插拔监听，以及设备创建和打开。当设备到来时，UsbDevManager会主动打开设备，不管是否打开成功，
***都将创建QSharedPointer<UsbDevice>对象***，并进行管理，当设备拔除时，释放其智能指针。因此，即使用户获取到设备，但由于***打开失败***，此时进行***读写安全但是会失败***。
当设备***打开成功***时，会打印该设备的配置的关键***描述符信息***：设备描述符，配置描述符，接口描述符，端点描述符，在UsbDevice::setConfiguration时，可以根据这些信息进行选择。

| 接口            | 说明                                                          |
|---------------|-------------------------------------------------------------|
| addMonitorId     | 添加需要监听的设备ID{PID, VID}                                       |
...
### 3. UsbDevice
USB设备对象，提供读写接口进行数据传输。用户通过UsbDevManager获取设备，UsbDevManager使用智能指针进行管理，用户不需要关心设备销毁。
