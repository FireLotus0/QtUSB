...
InterfaceDesc::InterfaceDesc(libusb_device *device, libusb_interface interface, int index)
...
void InterfaceDesc::resolveInfo() {
    ...
    for(int i = 0; i < interface.num_altsetting; i++) {
        auto alterInterface = new InterfaceAlter(device, interface.altsetting[i]);
