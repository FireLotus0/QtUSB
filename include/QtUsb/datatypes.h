...
struct QTUSB_API ActiveUSBConfig {
     ...
     * 例如：写入64字节数据，但是Windows平台上写入了65字节数据，即使设备最大包大小仅为64字节，
     * 此时设置discardBytes=1，每次分包写入时减去discardBytes，
     * 从而避免分包错误。当读取数据时，如果discardBytes > 0，
     * 则会将缓冲区大小设置为端点最大包大小sg + discardBytes，
     * 并且拷贝数据忽略后面的discardBytes个字节数据。目前只在单片机中断传输中使用
     * ，且可能跟设备有关！
     * */
     ...
     * 例如：单片机每次只能写入64字节，并且处理时间需要5ms，则设置cmdInterval = 5，
     * 当写入100字节的数据时，先写64字节，sleep(5), 再写36字节...
     * 目前只在中断传输和批量传输中使用，跟设备处理性能有关。
     */
